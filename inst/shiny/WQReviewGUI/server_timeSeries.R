...
output$timeseries_hoverinfo <- renderPrint({
        ...
        cat("Result flags:",
            names(subset(reports$resultFlagTable,PARM_CD == input$parmSel_TS & RECORD_NO == unique(hoverTable$RECORD_NO))[16:20])[which(sapply(subset(reports$resultFlagTable,PARM_CD == input$parmSel_TS & RECORD_NO == unique(hoverTable$RECORD_NO))[16:20], function(x)all(is.na(x))) == FALSE)],
            "\n");

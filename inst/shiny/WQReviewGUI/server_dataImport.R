observeEvent(input$dataDownload, {
        output$importWarning <- renderText(
                validate(
        ...
        tryCatch({
                ...
                withProgress(message="Data import",detail="Pulling data from NWIS",value=0,{
                        ...
                        },error = function(e){output$errors <- renderPrint(paste("ODBC message:",geterrmessage()))
                        stop(geterrmessage())
                        })
                        ...
                        if(!is.null(qw.data))
                        {
                                ...
                                output$samplesRetrieved <- renderText({
                                        paste("Samples retrieved:",length(unique(qw.data$PlotTable$RECORD_NO)))})
                        ...
                        incProgress(0.6,detail = "Populating Excel")
        ...
        if(!is.null(qw.data))
        {
                ...
                ###they can't run until after the data has been imported
                ###############################################

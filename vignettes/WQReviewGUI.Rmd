...
1. Copy results that have been reviewed from the Excel "Provisional data" tab to the "Reviewed Provisional data".
...
1. Copy everything but the column titles from the qwsample and qwresult Excel workbook tabs and paste them into seperate files using a text editor (Notepad, Notepad++, Wordpad) and save them in the project folder as qwsample and qwresult files.
...
* Select only one parameter from the dropdown list or in linked unapproved data table in Chrome.
...
These tabs will be present when the "Generate batch files" button is pressed in Chrome. See "Reviewed unapproved data" above. The tabs can copied and saved into text files for modifying DQI codes and comments with QWDATA. To regenerate the tabs if more results have been added to the "Reviewed Provisional data" tab, delete the current qwsample and qwresult tabs and hit the "Generate batch files" button again.

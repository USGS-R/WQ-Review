...
During data import, samples or results are auto-flagged following criteria developed by NAWQA for all water-quality data, and are defined [here](AutomatedDataflags.htm). For any flags comparing a current sample to historic data, the current sample is compared to all REVIEWED AND ACCEPTED data rather than prior samples in time. This is because data that have not been reviewed and accepted could themselves be problematic.
